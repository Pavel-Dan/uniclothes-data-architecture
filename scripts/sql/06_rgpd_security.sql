...
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'uniclothes_analyst') THEN
        CREATE ROLE uniclothes_analyst LOGIN PASSWORD 'analyst_read_2026';
    END IF;
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'uniclothes_dpo') THEN
        CREATE ROLE uniclothes_dpo LOGIN PASSWORD 'dpo_readonly_2026';
    END IF;
END
$$;
...
GRANT SELECT ON dwh.dim_product TO uniclothes_analyst;
GRANT SELECT ON dwh.dim_store TO uniclothes_analyst;
GRANT SELECT ON dwh.dim_channel TO uniclothes_analyst;
GRANT SELECT ON dwh.dim_date TO uniclothes_analyst;
GRANT SELECT ON dwh.fact_sales TO uniclothes_analyst;
...
BEGIN
    ...
    SET processed_at = CURRENT_TIMESTAMP,
        status = 'completed',
        notes = format('Supprime staging=%s, dwh=%s', v_staging, v_dwh)
